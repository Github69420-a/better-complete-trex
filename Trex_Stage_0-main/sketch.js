...
function draw()
{
  ...
  if (gameState === "play") {
    ...
    text(score, 550, 15);
    text("Score: ", 510, 15);
    text(Math.round(getFrameRate()), 50, 15);
    ...
    //Math.round(getFrameRate() / scoreDivider)
    //groundVelocity = -(2+(score/velocityDivider));
    ground1.velocityX = -(4 + 3* (score/100));
    //-(2+2*(score/velocityDivider))
    if (ground1.x < 0) {
...
function obstacles()
{
  var frameDelay = Math.round(random(60, 180));
  if (frameCount % frameDelay === 0) {
    obstacle = createSprite(500, 165, 50, 75);
    obstacleVelocity = -(4 + 3* (score/100));
    obstacle.velocityX = Math.round(obstacleVelocity);
    obstacle.scale = 0.6;
...
function reset() {
  ...
  restartSprite.visible = false;
  gameState = "play";
  trex.changeAnimation("hello", trex_running);
